...
const Dashboard = () => {
  ...
  // Fetch locations and cancer risk data
  const fetchLocationsData = async () => {
    setLoading(true);
    setError(null);
    try {
      const res = await axios.get('/api/locations-data');
  ...
  const fetchQuests = async () => {
    ...
    try {
      ...
      const [questsRes, userQuestsRes] = await Promise.all([
        ...
        userId ? axios.get(`/api/quests/user/${userId}`) : Promise.resolve({ data: [] })
      ]);
  ...
  const handleAssignQuest = async (questId) => {
    try {
      const userId = localStorage.getItem('ecotrackerUserId');
      await axios.post('/api/quests/assign', { userId, questId });
      fetchQuests();
    } catch (err) {
  ...
  const handleClaimReward = async (questId) => {
    try {
      const userId = localStorage.getItem('ecotrackerUserId');
      await axios.post('/api/quests/claim', { userId, questId });
      fetchQuests();
  ...
  const handleReport = async () => {
    ...
    try {
      ...
      await Promise.all(reportQuests.map(q =>
        axios.post('/api/quests/progress', { userId, questId: q.questId._id, increment: 1 })
      ));
      fetchQuests();
    } catch (err) {
